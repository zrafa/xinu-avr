...
// RAFA struct dateinfo Date = {0, FALSE, DATE_DST_AUTO,
// RAFA 		       {31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31},
// RAFA 		       {"J", "F", "Mar", "A", "May", "Jun", "Jul",
// RAFA 		        "Au", "S","O","N","D"},
// RAFA 		       {"S", "M", "T", "W", "Th", "F", "Sa"}};

		       // RAFA {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul",
// RAFA 		        "Aug", "Sep","Oct","Nov","Dec"},
// RAFA 		       {"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"}};
	...
	)
{
	...
	for (month=0 ; month<12 ; month++) {
		// RAFA tmp = Date.dt_msize[month] * SECPERDY;
		tmp = 1;
	...
	dst = FALSE;
	// RAFA  if (Date.dt_daylight == DATE_DST_ON) {
	// RAFA  	dst = TRUE;
	// RAFA  } else if (Date.dt_daylight == DATE_DST_AUTO) {
		...
		// RAFA mar1 = (jan1 + Date.dt_msize[0] + Date.dt_msize[1] +
						   //RAFA  isleap(year)) % 7;
...
// RAFA			if (day > Date.dt_msize[month]) {
				day = 1;
				month++;
...
// RAFA	sprintf(str, "%3s  %3s %2d  %2d:%02d:%02d %s %d",
// RAFA		Date.dt_dnam[dayofweek], Date.dt_mnam[month],
// RAFA		day, hour, minute, second, dst? dzones[TIMEZONE-5]:
// RAFA			zones[TIMEZONE-5], year);
	return OK;
}
